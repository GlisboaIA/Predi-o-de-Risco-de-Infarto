...
model.add_cpds(cpd_idade, cpd_historico, cpd_dor, cpd_tabagismo, cpd_estresse, cpd_hipertensao, cpd_risco, cpd_arritmia)
...
def mostrar_resultado():
    nome = combo.get()
    if nome:
        evidencias = pacientes[nome]
        risco = infer.query(['RiscoInfarto'], evidence=evidencias, show_progress=False)
        arritmia = infer.query(['Arritmia'], evidence=evidencias, show_progress=False)
        resultado_risco.config(text=f"Risco de Infarto (Alto): {risco.values[0]:.2f}")
        resultado_arritmia.config(text=f"Probabilidade de Arritmia (Sim): {arritmia.values[0]:.2f}")
        detalhes.config(text="\n".join([f"{k}: {v}" for k, v in evidencias.items()]))


janela = tk.Tk()
janela.title("Análise de Risco Cardiovascular com Redes Bayesianas")
janela.geometry("480x300")
janela.resizable(False, False)


tk.Label(janela, text="Selecione o paciente:", font=("Arial", 12)).pack(pady=5)
combo = ttk.Combobox(janela, values=list(pacientes.keys()), font=("Arial", 11))
combo.pack(pady=5)
tk.Button(janela, text="Calcular", command=mostrar_resultado).pack(pady=5)

# Resultados
resultado_risco = tk.Label(janela, text="", font=("Arial", 12, "bold"), fg="darkred")
resultado_risco.pack(pady=3)
resultado_arritmia = tk.Label(janela, text="", font=("Arial", 12, "bold"), fg="darkblue")
resultado_arritmia.pack(pady=3)

# Mostrar evidências
tk.Label(janela, text="Informações do Paciente:", font=("Arial", 11, "italic")).pack(pady=5)
detalhes = tk.Label(janela, text="", font=("Arial", 10))
